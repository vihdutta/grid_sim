...
class Agent():
        ...
        for episode in itertools.count():
            ...
            if is_training:
                if episode_reward > best_reward:
                    percent = (episode_reward - best_reward) / best_reward * 100 if best_reward != 0 else 0
                    log_message = f"{datetime.now().strftime(DATE_FORMAT)}: New best reward {episode_reward:0.1f} ({percent:.1f}%) at episode {episode}, saving model..."
                    print(log_message)
                ...
                if current_time - last_graph_update_time > timedelta(seconds=10):
                    self.save_graph(rewards_per_episode, epsilon_history, policy_dqn)
    ...
    def save_graph(self, rewards_per_episode, epsilon_history, policy_dqn):
        # Create a figure with a grid of 3 plots
        ...
        # Plot 3: Neural Network Architecture
        plt.subplot(133)  # plot on a 1 row x 3 col grid, at cell 3
        plt.title('Network Architecture')
        
        def draw_neural_network():
            # Get network dimensions
            input_size = policy_dqn.fc1.in_features
            hidden_size = policy_dqn.fc1.out_features
            output_size = policy_dqn.fc2.out_features
            
            # Define node positions
            layer_sizes = [input_size, hidden_size, output_size]
            layer_positions = [0, 1, 2]  # x-coordinates for layers
            
            # Clear axis ticks and set equal aspect ratio
            plt.gca().set_xticks([])
            plt.gca().set_yticks([])
            plt.gca().set_aspect('equal', adjustable='box')
            
            # Draw nodes for each layer
            for layer_idx, (n_nodes, x_pos) in enumerate(zip(layer_sizes, layer_positions)):
                # Calculate y positions for nodes in this layer
                y_positions = np.linspace(0, 1, n_nodes)
                
                # Draw nodes
                for y_pos in y_positions:
                    circle = plt.Circle((x_pos, y_pos), 0.02, color='skyblue', fill=True)
                ...
                if layer_idx < len(layer_sizes) - 1:
                    ...
                    for y1 in y_positions:
                        for y2 in next_y_positions:
                            plt.plot([x_pos, x_pos + 1], [y1, y2], 'gray', alpha=0.1)
            
            # Add layer labels
            plt.text(-0.1, -0.1, f'Input\n({input_size})', ha='center')
            plt.text(1, -0.1, f'Hidden\n({hidden_size})', ha='center')
            plt.text(2.1, -0.1, f'Output\n({output_size})', ha='center')
        ...
        # Save plots
        fig.savefig(self.GRAPH_FILE, dpi=300, bbox_inches='tight')
        plt.close(fig)
