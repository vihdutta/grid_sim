...
def draw_scene(screen, distances, CELLS, entities, entity_map, coins, grid_pixel_size, cell_pixel_size, grid_x, grid_y):
...
def draw_manhattan_distances(screen, entity, coin, distances, CELLS, cell_pixel_size, grid_x, grid_y):
    ...
    min_distance = min(up, down, left, right)
    ...
    for direction, (x, y, distance) in directions.items():
        if distance == min_distance:
            color = (0, 255, 0)  # green for the lowest distance
        else:
            color = (255, 0, 0)  # red for other distances
